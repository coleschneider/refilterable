import React, { createContext, useRef, useEffect, useContext, useState, useCallback, useDebugValue } from 'react';
...
var FiltersProvider = function (_a) {
    ...
    invariant(history, "refilterable: FiltersProvider was not passed a history instance. \n     Make sure you pass it via the history prop");
    var filterRegistry = useRef(createFilterRegistry());
    var locationObserver = useRef(createLocationObserver(history.location.search));
    useEffect(function () {
    ...
    return (React.createElement(filtersContext.Provider, { value: contextValue }, children));
};
...
function createFilter(paramName, config) {
    var _a;
    invariant(String(paramName || '').length > 0, "refilterable: param name cannot be empty");
...
function useFilter(filter) {
    ...
    invariant(filter, "refilterable: you called useFilter() without a filter.");
    // pull the context
    var context = useContext(filtersContext);
    invariant(context, "refilterable: Components that utilize the \"useFilter\" hook need to be wrapped with FiltersProvider.");
    ...
    if (typeof filter === 'string') {
        ...
        invariant((isFilterComposition(filter) || isFilterObject(filter)), "refilterable: you called useFilter and passed an invalid filter object.\n      Instead of constructing the configuration object on your own,\n      use createFilter() or composeFilters().");
    ...
    useEffect(function () {
        ...
        filters.forEach(function (filter) {
            invariant(!filterRegistry.isColliding(filter), "refilterable: you attempted to register conflicting configurations for the \"" + filter.paramName + "\" filter");
    ...
    var filterSetter = useCallback(function (nextValue, options) {
        ...
        function computeNextParams() {
            ...
            filters.forEach(function (_a) {
                ...
                if (!keysToSet.has(paramName)) {
                    ...
                    invariant(['string', 'undefined'].includes(typeof formattedValue), "refilterable: a custom formatter (" + paramName + ") produced a non-string value.\n            Make sure your formatter always returns a string or undefined");
                    // apply the new value
    ...
    useDebugValue('useFilter', function () { return paramNames.join(', '); });
    return [
        (singleFilterMode ?
...
function useReset(filter) {
    ...
    invariant(context, "refilterable: Components that utilize the \"useFilter\" hook need to be wrapped with FiltersProvider.");
    if (filter) {
        invariant((isFilterComposition(filter) || isFilterObject(filter)), "refilterable: you called useReset() and passed an invalid filter object.\n      Instead of constructing the configuration object on your own,\n      use createFilter() or composeFilters().");
...
function composeFilters(filters, validate) {
    var _a;
    invariant(filters && filters.length > 0, "refilterable: you called compose filters and didn't pass any filters ");
    invariant(filters.every(function (filter) { return isFilterObject(filter); }), "refilterable: composeFilters() was called with non-filter objects. \n    Use createFilter() to create filters");
    return _a = {
            filters: filters,
...
export { FiltersProvider, composeFilters, createFilter, useFilter, useReset };
